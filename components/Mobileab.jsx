import React from 'react'

const Mobileab = () => {
  return (
    <div className='md:hidden flex flex-col justify-center items-center min-h-full pt-16 bg-black'>
      <div>
       <div className='py-4 px-0 pl-2 flex flex-col justify-start items-start gap-4'>
          <span className="font-['Google Sans']  text-6xl font-bold leading-10 text-white">
              Explore 
          <br />
          </span>
          <span className="font-['Google Sans']  text-7xl font-bold  text-limee">"Startups"</span>          
       </div>
       <div className='p-6 pr-0 relative '>
                <div className="pb-4"> 
                  <span className="text-white text-start text-2xl font-normal font-['Google Sans'] leading-10">What are</span><span className="text-slate-500 text-3xl font-normal font-['Google Sans'] leading-10"> </span><span className="text-white text-3xl font-medium font-['Google Sans'] leading-10">Startups ?</span>
                </div>
                <div className="pb-4">      
                  <span className="text-white text-start text-2xl font-normal font-['Google Sans'] leading-10">Startup Lifecycle</span>
                </div >
                <div className="pb-4">
                  <span className="text-white text-start text-2xl font-normal font-['Google Sans'] leading-10">Market Research</span>
                </div>
                <div className="pb-4">
                  <span className="text-white text-start text-2xl font-normal font-['Google Sans'] leading-10" >How to Pitch?</span>
                </div >
                <div className="pb-4">
                  <span className="text-white  text-start text-2xl font-normal font-['Google Sans'] leading-10">How to get startup ideas?</span>
                </div>
                <div className='px-40 absolute right-15 top-20 z-[0]'>
                  <svg className='h-[150px]' viewBox="0 0 499 499" fill="none" xmlns="http://www.w3.org/2000/svg">
                    <g opacity="0.2">
                    <path d="M495.609 85.7849C495.597 84.9259 495.341 84.0881 494.87 83.3696C494.399 82.6511 493.734 82.0817 492.951 81.7281C440.819 57.8237 305.243 81.1589 223.585 136.433C209.038 146.198 195.371 157.212 182.737 169.351C161.978 163.418 140.56 160.913 121.051 165.295C65.9657 177.872 39.9552 233.347 31.0075 257.642C30.5095 258.971 30.3266 260.396 30.4729 261.807C30.6192 263.218 31.0909 264.576 31.8508 265.773C32.6108 266.971 33.6382 267.976 34.8524 268.71C36.0665 269.443 37.4343 269.885 38.8481 270L122.129 276.701C121.004 282.861 120.193 289.073 119.699 295.315C119.293 299.638 120.413 303.966 122.866 307.548L148.382 345.131C150.802 348.735 154.403 351.381 158.566 352.612C164.511 354.462 170.547 356.008 176.649 357.246L152.025 436.971C151.61 438.326 151.514 439.76 151.745 441.159C151.976 442.557 152.528 443.884 153.358 445.033C154.188 446.183 155.272 447.125 156.527 447.785C157.782 448.445 159.172 448.806 160.59 448.838C186.404 449.642 247.674 446.101 279.668 399.594C290.978 383.105 296.63 362.399 298.862 341.035C314.857 333.815 330.193 325.215 344.695 315.334C426.548 260.204 497.726 145.744 495.609 85.7849ZM291.85 220.601C286.783 213.125 284.048 204.314 283.99 195.283C283.931 186.252 286.553 177.407 291.522 169.866C296.492 162.325 303.586 156.427 311.908 152.919C320.23 149.41 329.405 148.448 338.274 150.155C347.142 151.862 355.305 156.161 361.73 162.508C368.155 168.855 372.554 176.964 374.369 185.811C376.185 194.658 375.335 203.845 371.929 212.209C368.523 220.573 362.712 227.739 355.233 232.801C350.271 236.166 344.695 238.52 338.823 239.728C332.952 240.936 326.899 240.974 321.012 239.841C315.125 238.708 309.52 236.426 304.516 233.125C299.512 229.823 295.208 225.568 291.85 220.601Z" fill="#F4F4F4"/>
                    <path d="M141.842 367.536C135.736 371.679 127.142 372.098 116.931 371.905C94.0007 371.415 78.0879 348.456 86.3961 326.779C89.5563 318.563 96.3539 307.502 99.6953 305.239C100.423 304.757 100.981 304.057 101.289 303.239C101.596 302.422 101.639 301.528 101.409 300.685C101.18 299.842 100.691 299.092 100.012 298.543C99.3328 297.994 98.4977 297.672 97.6254 297.625C84.9176 296.754 72.2837 300.16 61.7353 307.3C35.5463 325.051 17.777 406.202 17.777 406.202C17.777 406.202 99.7729 419.833 125.962 402.082C136.533 394.95 144.396 384.467 148.283 372.321C149.38 368.483 145.073 365.243 141.842 367.536Z" fill="#F4F4F4"/>
                    </g>
                  </svg>              
                </div>
       </div>
      </div>
      <div>
       <div className='py-4 px-0 pr-2 flex flex-col justify-end items-end gap-4'>
          <span className="font-['Google Sans']  text-6xl font-bold leading-10 text-white">
              Explore 
          <br />
          </span>
          <span className="font-['Google Sans']  text-7xl font-bold  text-limee">"Design"</span>          
       </div>
       <div className='p-6 pr-4 relative flex flex-col justify-end items-end'>
                <div className="pb-4"> 
                  <span className="text-white text-start text-2xl font-normal font-['Google Sans'] leading-10">What is </span><span className="text-slate-500 text-3xl font-normal font-['Google Sans'] leading-10"> </span><span className="text-white text-3xl font-medium font-['Google Sans'] leading-10">Design ?</span>
                </div>
                <div className="pb-4">      
                  <span className="text-white text-start text-2xl font-normal font-['Google Sans'] leading-10">Design as a Career</span>
                </div >
                <div className="pb-4">
                  <span className="text-white text-start text-2xl font-normal font-['Google Sans'] leading-10">Elements of Graphic D</span>
                </div>
                <div className="pb-4">
                  <span className="text-white text-start text-2xl font-normal font-['Google Sans'] leading-10" >How to Pitch?</span>
                </div >
                <div className="pb-4">
                  <span className="text-white  text-start text-2xl font-normal font-['Google Sans'] leading-10">How to get startup ideas?</span>
                </div>
                <div className='pr-40 absolute left-15 top-12 z-'>
                  <svg className='h-[150px]' viewBox="0 0 421 421" fill="none" xmlns="http://www.w3.org/2000/svg">
                    <g opacity="0.2">
                    <path d="M210.501 215.763L61.3965 123.669L210.501 31.5757L359.605 123.669L210.501 215.763Z" fill="#F4F4F4"/>
                    <path opacity="0.25" d="M210.499 389.426V215.763L359.603 123.669V297.332L210.499 389.426Z" fill="#F4F4F4"/>
                    <path opacity="0.5" d="M210.501 389.426L61.3965 297.332V123.669L210.501 215.763V389.426Z" fill="#F4F4F4"/>
                    </g>
                  </svg>
                </div>
       </div>
      </div>
      <div>      
       <div className='py-4 px-0 pl-2 flex flex-col justify-start items-start gap-4'>
          <span className="font-['Google Sans']  text-6xl font-bold leading-10 text-white">
              Explore 
          <br />
          </span>
          <span className="font-['Google Sans']  text-7xl font-bold  text-limee">"Tech"</span>          
       </div>
       <div className='p-6 pr-0 relative'>
        <div className="pb-4"> 
                  <span className="text-white text-2xl text-start font-normal font-['Google Sans'] leading-10">Intro to</span><span className="text-slate-500 text-5xl font-normal font-['Google Sans'] leading-10"> </span><span className="text-white text-3xl font-medium font-['Google Sans'] leading-10">Computer Science</span>
                </div>
                <div className="pb-4">      
                  <span className="text-white text-2xl text-start font-normal font-['Google Sans'] leading-10">Software Development Lifecycle</span>
                </div >
                <div className="pb-4">
                  <span className="text-white text-2xl text-start font-normal font-['Google Sans'] leading-10">Various Computer Languages</span>
                </div> 
                <div className="pb-4">
                  <span className="text-white text-2xl  text-start font-normal font-['Google Sans'] leading-10" >Generative AI</span>
                </div >
                <div className="pb-4">
                  <span className="text-white text-2xl text-start font-normal font-['Google Sans'] leading-10">Open Source and Git</span>
                </div>
                <div className='px-40 absolute right-15 top-20 z-0'>
                  <svg className='lg:h-[333px] h-[150px]' viewBox="0 0 335 335" fill="none" xmlns="http://www.w3.org/2000/svg">
                      <path opacity="0.2" d="M132.944 46.5556L167.5 12M98.3889 236.611L236.611 98.3889M167.5 323L202.056 288.444M245.25 288.444C256.706 288.444 267.693 283.894 275.793 275.793C283.894 267.693 288.444 256.706 288.444 245.25C288.444 233.794 283.894 222.807 275.793 214.707C267.693 206.606 256.706 202.056 245.25 202.056C233.794 202.056 222.807 206.606 214.707 214.707C206.606 222.807 202.056 233.794 202.056 245.25C202.056 256.706 206.606 267.693 214.707 275.793C222.807 283.894 233.794 288.444 245.25 288.444ZM89.75 132.944C101.206 132.944 112.193 128.394 120.293 120.293C128.394 112.193 132.944 101.206 132.944 89.75C132.944 78.2941 128.394 67.3074 120.293 59.2069C112.193 51.1064 101.206 46.5556 89.75 46.5556C78.2941 46.5556 67.3074 51.1064 59.2069 59.2069C51.1064 67.3074 46.5556 78.2941 46.5556 89.75C46.5556 101.206 51.1064 112.193 59.2069 120.293C67.3074 128.394 78.2941 132.944 89.75 132.944ZM55.1944 323C66.6503 323 77.637 318.449 85.7375 310.349C93.8381 302.248 98.3889 291.261 98.3889 279.806C98.3889 268.35 93.8381 257.363 85.7375 249.262C77.637 241.162 66.6503 236.611 55.1944 236.611C43.7386 236.611 32.7519 241.162 24.6514 249.262C16.5508 257.363 12 268.35 12 279.806C12 291.261 16.5508 302.248 24.6514 310.349C32.7519 318.449 43.7386 323 55.1944 323ZM279.806 98.3889C291.261 98.3889 302.248 93.8381 310.349 85.7375C318.449 77.637 323 66.6503 323 55.1944C323 43.7386 318.449 32.7519 310.349 24.6514C302.248 16.5508 291.261 12 279.806 12C268.35 12 257.363 16.5508 249.262 24.6514C241.162 32.7519 236.611 43.7386 236.611 55.1944C236.611 66.6503 241.162 77.637 249.262 85.7375C257.363 93.8381 268.35 98.3889 279.806 98.3889Z" stroke="#E9E9E9" stroke-width="23.9231" stroke-linecap="round" stroke-linejoin="round"/>
                  </svg>              
                </div>
        </div>
      </div>
      
    </div>
  )
}

export default Mobileab
